        ...
        if include_considered and self.child_sum is not None:
            ...
            if (not consider_exception) or (not action in exception):
                ...
                condition = encode(AND(action.presence, self.filter_func.evaulate(action)), assumption=assumption,
                                   ...
                                   disable=disable, unsat_mode=unsat_mode)
            ...
            for action in self.input_type.snap_shot:
                if (not consider_exception) or (not action in exception):
                    if action in self.encoded_actions:
                        ...
                        if self.pos:
                            encode(cond, unsat_mode=unsat_mode)
                            starting += res_pos
                        if self.neg:
                            encode(neg_cond, unsat_mode=unsat_mode)
                        ...
                        if self.neg:
                            ...
                            under_ite_result = ITE(
                                encode(neg_cond, assumption=assumption, include_new_act=include_new_act,
                                       exception=exception,
                                       disable=disable, unsat_mode=unsat_mode), Int(0), value)
...
class Summation(Arth_Operator):
    ...
    def encode(self, assumption=False, include_new_act=False, exception=None, disable=None, proof_writer=None, unsat_mode=False):
        ...
        if disable:
            under_value, _ = action.under_encode(assumption, include_new_act, exception, disable, unsat_mode=unsat_mode)
            return under_value
        else:
